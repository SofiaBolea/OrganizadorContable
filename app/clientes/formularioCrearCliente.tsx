        ...
        <form onSubmit={handleOnSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input 
              ...
              required
              className="bg-[#e9e8e0] p-4 rounded-full outline-none placeholder:text-gray-500 text-gray-700"
            />
            <input 
              name="cuit" 
              placeholder="CUIT" 
          ...
          {/* Sección Asistentes */}
          <div className="bg-[#ecebe4] rounded-[30px] p-6">
            <div className="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
              <div className="flex items-center gap-2 text-gray-600">
                <span className="font-medium">Asistentes asignados</span>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
              </div>
              
              <div className="flex items-center gap-3">
                <span className="text-gray-600 text-sm">Asignar a todos</span>
                <button 
                  type="button"
                  onClick={handleToggleAll}
                  className={`w-12 h-6 rounded-full transition-colors relative ${asignarTodos ? 'bg-[#f4a28c]' : 'bg-gray-300'}`}
                >
            ...
            {/* Buscador interno (opcional pero recomendado) */}
            <input 
              type="text" 
              placeholder="Buscar asistente..." 
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-transparent mb-4 text-sm outline-none italic text-gray-500"
            />
          ...
          <div className="flex justify-between items-center pt-4">
            <button 
              ...
              className="bg-[#f4a28c] text-[#8e4a3a] px-12 py-3 rounded-2xl font-bold hover:opacity-90 transition-opacity"
            >
              Cancelar
            </button>
            <button 
              type="submit"
              disabled={loading}
              className="bg-[#98c18c] text-[#3e5a34] px-12 py-3 rounded-2xl font-bold hover:opacity-90 transition-opacity disabled:opacity-50"
            >
